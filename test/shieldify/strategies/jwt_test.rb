...
class JwtStrategyTest < ActionDispatch::IntegrationTest
  ...
  test 'successful JWT authentication' do
    get '/warden-authenticated-show', headers: { 'Authorization' => "Bearer #{@token}" }
  ...
  test 'failed JWT authentication with invalid token' do
    get '/warden-authenticated-show', headers: { 'Authorization' => "Bearer invalidtoken" }
  ...
  test 'no authenticated action' do
    get '/warden-no-authenticated-index'
    assert_response :success
